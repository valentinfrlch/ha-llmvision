...
class Timeline(CalendarEntity):
    ...
    def __init__(self, hass: HomeAssistant, config_entry: ConfigEntry):
        ...
        self._file_path = self.hass.config.path(f"{DOMAIN}/snapshots")
    ...
    async def _migrate(self):
        ...
        # v3 -> v4: Migrate image paths to /llmvision/snapshots from /www/llmvision
        try:
            ...
            await self.hass.loop.run_in_executor(
                ...
                partial(
                    os.makedirs,
                    self.hass.config.path("llmvision/snapshots"),
                    exist_ok=True,
                ),
            )
            src_dir = self.hass.config.path("www/llmvision")
            dst_dir = self.hass.config.path("llmvision/snapshots")
            ...
            async with aiosqlite.connect(self._db_path) as db:
                await db.execute(
                    """
                    UPDATE events SET key_frame = REPLACE(key_frame, '/www/llmvision', '/llmvision/snapshots')
                """
                )
                await db.commit()
        except aiosqlite.Error as e:
