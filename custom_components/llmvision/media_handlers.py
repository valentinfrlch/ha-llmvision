...
class MediaProcessor:
    def __init__(self, hass, client):
        ...
        self.base64_images = []
        self.filenames = []
        self.path = self.hass.config.path(f"{DOMAIN}/snapshots/")
        self.key_frame = ""
    ...
    async def _expose_image(self, frame_name, image_data, uid, frame_path=None):
        # ensure /llmvision/snapshots dir exists
        await self.hass.loop.run_in_executor(
            None,
            partial(os.makedirs, self.hass.config.path(f"{DOMAIN}/snapshots"), exist_ok=True),
        )
        if self.key_frame == "":
            filename = self.hass.config.path(f"{DOMAIN}/snapshots/{uid}-{frame_name}.jpg")
