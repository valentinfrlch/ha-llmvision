...
def create_benchmark_visualization(df: pd.DataFrame):
    ...
    category_order = ['GPT-4', 'Claude 3.7', 'Claude 3.5',
                      'Claude 3', 'Gemini 2.0', 'Gemini 1.5']
    ...
    colors = {'GPT-4': '#00cbbf', 'Claude 3': '#d97857',
              'Gemini 1.5': '#5da9ff', 'Gemini 2.0': '#5da9ff', 'Claude 3.5': '#d4a27f', 'Claude 3.7': '#d4a27f', 'Other': 'gray'}

    for category, group_df in df.groupby('Category'):
        if category not in ['GPT-4', 'Claude 3', 'Claude 3.5', 'Claude 3.7', 'Gemini 2.0', 'Gemini 1.5', 'Other']:
            continue
    ...
    fig.update_layout(title={'text': 'Performance vs Cost of Cloud-Based Models in LLM Vision',
                             'font': {'size': 50}},
                      xaxis_title='$/1M Input Tokens', yaxis_title='MMMU Score',
    ...
    open_source_colors = {'Gemma': '#5da9ff', 'Llama': '#0081fb',
                          'LLaVA': '#ff7f0e', 'MiniCPM': '#2ca02c', 'Qwen': 'gray', 'Other': 'gray'}
    ...
    fig_open_source.update_layout(title={'text': 'Performance vs Model Size of Open Source Models in LLM Vision',
                                         'font': {'size': 50}},
                                  xaxis_title='Model Size (Parameters)', yaxis_title='MMMU Score',
                                  paper_bgcolor='#0d1117', plot_bgcolor='#161b22',
                                  font=dict(color='white',
                                            family='Product Sans', size=25),
                                  xaxis=dict(color='white', linecolor='grey',
                                             showgrid=False, zeroline=False),
                                  yaxis=dict(color='white', linecolor='grey',
                                             showgrid=False, zeroline=False))
